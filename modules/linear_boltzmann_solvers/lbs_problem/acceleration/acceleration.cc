...
void
CheckBlockwiseUniformDensities(const LBSProblem& lbs_problem,
                                        const std::string& acceleration_name)
{
  ...
  mpi_comm.all_reduce(
    local_min.data(), local_min.size(), global_min.data(), mpi::op::min<double>());
  mpi_comm.all_reduce(
    local_max.data(), local_max.size(), global_max.data(), mpi::op::max<double>());
  ...
  for (size_t i = 0; i < block_ids.size(); ++i)
  {
    ...
    const auto tol = 1.0e-12 * std::max(1.0, std::max(std::fabs(global_min[i]), std::fabs(global_max[i])));
    if (std::fabs(global_max[i] - global_min[i]) > tol)
    {
      std::stringstream msg;
      msg << acceleration_name
          << " requires uniform density per block ID. Block " << block_ids[i]
