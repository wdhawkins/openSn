            ...
            double* saved_psi)
{
  ...
  if (cell.num_nodes == 0)
    return;
  const auto* flud_index = [&]() -> const std::uint64_t*
  {
    if constexpr (t == SweepType::AAH)
      return args.flud_indices[angle_idx];
    else
      return args.flud_index;
  }();
  auto [cell_edge_data, _] = GetCellDataIndex(flud_index, cell_local_idx);
