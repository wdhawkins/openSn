...
__global__ void
UnsaturatedKernelMain(Arguments args, const std::uint32_t* level, double* saved_psi)
{
...
/// Kernel performing the sweep
__global__ void
UnsaturatedKernelOptimized(Arguments args, const std::uint32_t* level, double* saved_psi)
{
  // reference to indexes
  const unsigned int& cell_idx = blockIdx.x;
  const unsigned int& angle_idx = threadIdx.y;
...
/// Kernel performing the sweep
__global__ void
SaturatedKernelOptimized(Arguments args, const std::uint32_t* level, double* saved_psi)
{
  // reference to indexes
  const unsigned int& cell_idx = blockIdx.x;
  // get cell view
