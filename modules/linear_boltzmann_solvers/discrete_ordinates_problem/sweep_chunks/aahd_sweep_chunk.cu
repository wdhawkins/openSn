...
{
  const char* env =
    std::getenv("OPENSN_AAH_PERSISTENT_SWEEP"); // NOLINT(concurrency-mt-unsafe)
  // Enabled by default; set OPENSN_AAH_PERSISTENT_SWEEP=0 to disable
...
AAHDSweepChunk::Sweep(AngleSet& angle_set)
{
  ...
  {
    const std::size_t sz = level.size();
    actual_kernel_launch_count_.fetch_add(1, std::memory_order_relaxed);
    total_level_cell_count_.fetch_add(sz, std::memory_order_relaxed);
    AtomicStoreMax(max_level_cell_count_, sz);
  }
...
#if defined(__NVCC__) || defined(__HIPCC__)
  if (UsePersistentSweep())
  {
    // One cooperative kernel per sweep pass per angle set.  The kernel loops over
    // all levels internally, using cg::grid_group::sync() between levels to enforce
    // the level-to-level data dependency without returning to the host dispatcher.
    //
    // Grid is sized to max_level_size (not the per-level size), so some threads
    // idle on smaller levels but all still participate in the grid sync.
    const std::uint32_t max_cells = spds.GetMaxLevelSize();
    const unsigned int grid_size_y = (max_cells + block_size_y - 1) / block_size_y;
    ...
    // Pack kernel arguments for GPU_API(LaunchCooperativeKernel) (void** interface).
    auto a0 = args;
    auto a1 = spds.GetDeviceLevelFlat();
    auto a2 = spds.GetDeviceLevelOffsets();
    auto a3 = spds.GetDeviceLevelSizes();
    auto a4 = static_cast<std::uint32_t>(levels_in_angle_set);
    auto a5 = level_node_data;
    auto a6 = level_cell_starts;
    auto a7 = saved_psi;
    void* kargs[] = {&a0, &a1, &a2, &a3, &a4, &a5, &a6, &a7};

    GPU_API(LaunchCooperativeKernel)(
      reinterpret_cast<const void*>(
        gpu_kernel::SweepPersistentKernel<gpu_kernel::SweepType::AAH>),
      grid_dim,
      block_dim,
      kargs,
      0,
  ...
  // Fallback: one kernel launch per level.
