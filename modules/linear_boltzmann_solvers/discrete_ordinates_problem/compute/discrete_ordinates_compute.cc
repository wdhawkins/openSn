...
double
ComputeRelativeBalance(const double gain, const double balance)
{
  if (gain != 0.0)
    return std::abs(balance) / std::abs(gain);

  return balance == 0.0 ? 0.0 : std::numeric_limits<double>::infinity();
}
...
ComputeBalanceTable(DiscreteOrdinatesProblem& do_problem, double scaling_factor)
{
        ...
        if (cell_volume > 0.0)
        {
          ...
          for (size_t r = 0; r < charged_ranges.size(); ++r)
          {
            ...
            for (unsigned int g = g_begin; g < g_end; ++g)
            {
              ...
              local_csda_energy_deposition += edge_energy_loss * energy_space_current * cell_volume;
  ...
  if (options.csda_enabled)
  {
    ...
    csda_particle_relative_balance =
      ComputeRelativeBalance(global_production + global_in_flow, csda_particle_balance.value());
    ...
    csda_energy_relative_balance = ComputeRelativeBalance(
