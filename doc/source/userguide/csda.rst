...
* ``csda_particle_relative_balance``: the absolute
  ``csda_particle_balance`` divided by the absolute particle gain
  ``production_rate + inflow_rate``,
* ``csda_energy_deposition_rate``: the integrated CSDA energy-deposition rate.

The energy balance entries also include ``csda_energy_balance`` and
``csda_energy_relative_balance``. The latter is the absolute energy balance
divided by the absolute energy gain
``csda_energy_production_rate + csda_energy_inflow_rate``. A relative balance
is zero when both its gain and residual are zero, and infinite when its gain is
zero but its residual is nonzero.

No additional solver normalization is applied to these values.
