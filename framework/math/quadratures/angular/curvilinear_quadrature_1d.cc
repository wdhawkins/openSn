...
GLQuadrature1DSpherical::GLQuadrature1DSpherical(unsigned int Npolar,
                                                 unsigned int scattering_order,
                                                 bool verbose,
                                                 OperatorConstructionMethod method)
  : CurvilinearQuadrature1D(1, scattering_order, method)
{
  if (Npolar % 2 != 0)
    throw std::invalid_argument("GLQuadrature1DSpherical: Npolar must be even.");
...
void
GLQuadrature1DSpherical::Initialize(unsigned int Npolar, const bool verbose)
{
  ...
  if (polar_quad.weights.empty())
    throw std::invalid_argument("GLQuadrature1DSpherical: "
  ...
  for (size_t p = 0; p < polar_quad.weights.size(); ++p)
  {
    ...
    const auto theta = std::acos(pol_abs);

    const auto weight = pol_wei;
    const auto abscissa = QuadraturePointPhiTheta(0, theta);
    ...
    polar_ang_.emplace_back(theta);
  }
  weights_.shrink_to_fit();
  abscissae_.shrink_to_fit();
  omegas_.shrink_to_fit();
  polar_ang_.shrink_to_fit();

  // Map of direction indices
  map_directions_.clear();
  for (size_t p = 0; p < polar_quad.weights.size(); ++p)
    map_directions_.emplace(p, std::vector<unsigned int>{static_cast<unsigned int>(p)});
  ...
  if (verbose)
  {
    ...
    log.Log() << "curvilinear quadrature : spherical" << std::endl;
...
void
GLQuadrature1DSpherical::InitializeParameters()
{
...
void
GLQuadrature1DSpherical::MakeHarmonicIndices()
{
